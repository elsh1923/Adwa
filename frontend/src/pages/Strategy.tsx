    ...
    const fetchStrategies = async () => {
      ...
      try {
        ...
        const mappedStrategies = data.map((item: any) => ({
          strategy_name: typeof item.strategy_name === 'object' ? (item.strategy_name['en']) : item.strategy_name,
    ...
    <div style={{ padding: '3rem 0' }}>
      ...
      {error && (
        <div style={{ textAlign: 'center', margin: '4rem 0', color: '#ff6b6b' }}>
          Failed to load strategies. Ensure the backend RAG server is running.
        </div>
      )}
      ...
      <div style={{ marginTop: '2rem', background: 'rgba(212,175,55,0.05)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 18, padding: '1.5rem 2rem', display: 'flex', gap: '1.25rem', alignItems: 'flex-start' }}>
        ...
        <div>
          <h4 style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--text)', marginBottom: '0.4rem', fontFamily: 'inherit' }}>Knowledge Base Integration</h4>
          <p style={{ color: 'var(--text-dim)', lineHeight: 1.8, fontSize: '0.9rem', fontFamily: 'inherit' }}>
            This page is now populated entirely by the custom JSON models hosted by your RAG Node backend.
          </p>
        </div>
      </div>
